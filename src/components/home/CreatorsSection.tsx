    ...
    try {
      ...
      const creatorsWithStats = await Promise.all(
        (profiles || []).map(async (profile) => {
          ...
          // Get reviews data (for rating and count)
      ...
      // Filter and sort creators by review count then by average rating
      const filteredCreators = creatorsWithStats
      ...
      <div className="container mx-auto px-4">
        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {[...Array(8)].map((_, i) => (
              <Card key={i} className="animate-pulse h-[420px]">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {creators.map((creator) => (
              <Card key={creator.id} className="group hover:shadow-2xl transition-all duration-300 border-0 bg-white overflow-hidden h-[420px] flex flex-col">
                {/* Upper half with gradient background and profile image */}
                <div className="h-48 bg-gradient-to-br from-orange-500 to-purple-600 relative flex items-center justify-center p-4">
                  <div className="absolute inset-0 bg-black/10"></div>
                  <div className="relative w-24 h-24 rounded-lg overflow-hidden ring-4 ring-white/30 shadow-xl">
                    {creator.avatar_url ? (
                      <img
                ...
                <CardContent className="p-5 flex-1 flex flex-col">
                  <div className="space-y-3">
                      ...
                      {creator.average_rating > 0 && (
                        <div className="flex items-center gap-1 bg-white rounded-full px-2 py-1 shadow-sm">
                          ...
                          <span className="text-sm font-semibold text-gray-800">
                            ...
                            <span className="text-xs text-gray-500 ml-1">({creator.review_count})</span>
                          </span>
                        </div>
                      )}
                    </div>
                    ...
                    <p className="text-sm text-gray-600 line-clamp-2 min-h-[2.5rem]">
                      {creator.bio || 'Expert instructor passionate about sharing knowledge.'}
                    </p>
                    
                    <div className="flex items-center justify-between text-xs text-gray-500 mt-2">
                      <div className="flex items-center gap-1">
                        <BookOpen className="h-3 w-3 text-orange-500" />
                        <span>{creator.total_courses} {creator.total_courses === 1 ? 'course' : 'courses'}</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <Calendar className="h-3 w-3 text-purple-500" />
                        <span>{creator.total_events} {creator.total_events === 1 ? 'event' : 'events'}</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <Users className="h-3 w-3 text-blue-500" />
                        <span>{creator.total_students.toLocaleString()}</span>
                      </div>
                    </div>
                      ...
                      <div className="flex flex-wrap gap-2 mt-3">
                        {creator.specialties.slice(0, 2).map((specialty) => (
                          <Badge 
                            key={specialty} 
                            variant="secondary" 
                            className="text-xs px-2 py-1 bg-gradient-to-r from-orange-100 to-purple-100 text-gray-700 border-0"
                          >
                            {specialty}
                          </Badge>
                        ))}
                  ...
                  <Button 
                    asChild
                    className="w-full mt-auto bg-gradient-to-r from-orange-500 to-purple-600 hover:from-orange-600 hover:to-purple-700 text-white border-0 shadow-lg hover:shadow-xl transition-all duration-300"
                  >
