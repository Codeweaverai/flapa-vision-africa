...
import { Bot, Sparkles, BookOpen, Clock, Users, Zap, CheckCircle, ArrowRight, Play, Star, FileText, Target, GraduationCap, Coins, Gift, History, ChevronRight, RefreshCw, AlertTriangle, Calendar, Send, Brain, Lightbulb, Rocket, Search, Cpu, Layers } from 'lucide-react';
  ...
  const createFullCourse = async () => {
    ...
    try {
      ...
      if (data?.success) {
        ...
        toast.success('Course created successfully with multi-agent system!');
        ...
        {step === 'creating' && !functionError && (
          <Card className="border-0 shadow-2xl bg-white/80 backdrop-blur-sm text-center">
            <CardContent className="py-16">
              ...
              <h3 className="text-3xl font-bold mb-4 text-gray-800">Multi-Agent Course Creation</h3>
              <p className="text-gray-600 mb-6 max-w-md mx-auto text-lg">
                Lumo AI agents are collaborating to create your complete course...
              </p>
              
              {/* Enhanced Progress Indicators */}
              <div className="max-w-md mx-auto space-y-4">
                {[
                  { task: 'Structure Agent: Designing course framework', agent: '🏗️' },
                  { task: 'Content Agent: Creating engaging lessons', agent: '📚' },
                  { task: 'Quiz Agent: Developing assessments', agent: '❓' },
                  { task: 'Exam Agent: Building final exam', agent: '📝' },
                  { task: 'Image Agent: Generating thumbnail', agent: '🖼️' },
                  { task: 'Manager Agent: Assembling course', agent: '🎯' }
                ].map((item, index) => (
