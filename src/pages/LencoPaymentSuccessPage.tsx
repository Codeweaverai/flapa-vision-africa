...
import { Check, ArrowRight, Calendar, BookOpen, Ticket, Sparkles, Gift, Loader2, AlertCircle, Clock, Home, Building } from 'lucide-react';
import { supabase } from '@/lib/supabaseClient';
import { toast } from 'sonner';
import { useAuth } from '@/contexts/AuthContext';

const LencoPaymentSuccessPage = () => {
  ...
  useEffect(() => {
    const verifyPayment = async () => {
      if (!reference) {
        toast.error('Invalid payment return URL - missing payment reference');
        navigate('/');
        return;
      }
      ...
      try {
        ...
        const { data, error } = await supabase.functions.invoke('lenco-create-verify', {
          body: {
            reference: reference,
            order_id: orderId
          }
        ...
        if (data.success) {
          setOrderDetails(data.verification_data);
  ...
  const startPolling = (paymentReference: string, orderId: string) => {
    const pollInterval = setInterval(async () => {
      try {
        const { data } = await supabase.functions.invoke('lenco-create-verify', {
          body: { 
        ...
        if (data.success && data.payment_status === 'successful') {
    ...
    setTimeout(() => {
      ...
      if (paymentStatus === 'pending') {
        toast.info('Payment is taking longer than expected. Please check your email for confirmation.');
      }
    }, 600000); // 10 minutes
  };
          ...
          <div className="max-w-2xl mx-auto px-4 w-full">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center mt-6">
              {paymentStatus === 'pending' && (
                <Button
                  onClick={() => reference && startPolling(reference, orderId || '')}
                  disabled={loading}
                  variant="outline"
                >
                ...
                <CardContent className="pt-6">
                  <div className="text-center text-blue-800">
                    ...
                    <p className="text-sm mt-1">
                      Payments via Lenco can take a few moments to process. 
            ...
            <Card className="text-center border-0 shadow-2xl bg-white/90 backdrop-blur-sm">
              <CardHeader className="pb-6">
                ...
                <div className="flex justify-center items-center gap-2 text-sm text-gray-600">
                  ...
                  <span>Paid securely with Lenco</span>
                </div>
              </CardHeader>
...
export default LencoPaymentSuccessPage;
