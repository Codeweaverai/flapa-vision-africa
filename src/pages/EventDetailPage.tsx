  ...
  <Card className="shadow-lg">
    ...
    <CardContent>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {recommendedEvents
          .filter(recEvent => new Date(recEvent.start_time) > new Date()) // Only upcoming events
          .sort((a, b) => {
            // Smart sorting algorithm: prioritize similar event type and higher ratings
            const currentEventType = event?.event_type || '';
            
            // Score events based on relevance
            const scoreA = calculateRelevanceScore(a, currentEventType);
            const scoreB = calculateRelevanceScore(b, currentEventType);
            
            return scoreB - scoreA; // Sort by relevance score descending
          })
          ...
          .map((recEvent) => (
            <Card key={recEvent.id} className="group overflow-hidden bg-white shadow-lg hover:shadow-2xl transition-all duration-300 border-0 hover:scale-[1.02] cursor-pointer"
              ...
              <div className="relative overflow-hidden">
                ...
                {recEvent.average_rating > 0 && (
                  <div className="absolute top-12 left-3">
                    <Badge className="bg-blue-500 hover:bg-blue-600 text-white border-0 flex items-center gap-1 text-xs">
                      <Star className="h-3 w-3 fill-current" />
                      <span>{recEvent.average_rating.toFixed(1)}</span>
                    </Badge>
                  </div>
                )}
